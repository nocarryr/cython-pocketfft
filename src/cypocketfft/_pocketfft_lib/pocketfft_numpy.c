...
execute_real_forward(PyObject *a1, double fct)
{
    ...
    int ndim = PyArray_NDIM(data);                              // ndim = data.ndim
    const npy_intp *odim = PyArray_DIMS(data);                  // odim = data.shape
    int npts = odim[ndim - 1];                                  // npts = nfft = data.shape[-1] (last axis)
    npy_intp *tdim=(npy_intp *)malloc(ndim*sizeof(npy_intp));
    if (!tdim)
      { Py_XDECREF(data); return NULL; }
    for (int d=0; d<ndim-1; ++d)
      tdim[d] = odim[d];
    tdim[ndim-1] = npts/2 + 1;                                  // tdim = data.shape
                                                                // tdim[-1] = npts/2 + 1
                                                                // same as input, but last axis == npts / 2 + 1 (expected for rfft)
    PyArrayObject *ret = (PyArrayObject *)PyArray_Empty(ndim,   // ret = np.empty(shape=tdim, dtype=np.complex128)
            tdim, PyArray_DescrFromType(NPY_CDOUBLE), 0);
    free(tdim);
    if (!ret) fail=1;
    if (!fail) {
      int rstep = PyArray_DIM(ret, PyArray_NDIM(ret) - 1)*2;    // rstep = ret.shape[-1] * 2

      int nrepeats = PyArray_SIZE(data)/npts;                   // nrepeats = data.size / npts   (total input size / nfft)
                                                                // if data.size == npts:   #(true if 1-d)
                                                                //     nrepeats = 1
      double *rptr = (double *)PyArray_DATA(ret),               // cast `ret` to double (from NPY_CDOUBLE). this should make
                                                                //     the real and imag components interleaved
             *dptr = (double *)PyArray_DATA(data);              // cast `data` to double (which it already is)
      ...
      if (!fail)
        for (int i = 0; i < nrepeats; i++) {
            rptr[rstep-1] = 0.0;                                // sets last element of the pointer to 0
                                                                // this should be the same as:
                                                                // ret.imag[i,-1] = 0

            memcpy((char *)(rptr+1), dptr, npts*sizeof(double));// copy data into ret starting at ret.imag[:,0]
                                                                // then pass rptr to rfft_forward with the same offset

            if (rfft_forward(plan, rptr+1, fct)!=0) {fail=1; break;}
                                                                // rfft_forward performs the operation in-place, not sure why the offset is done

            rptr[0] = rptr[1];                                  // ret.real[0] = ret.imag[0]
            rptr[1] = 0.0;                                      // ret.imag[0] = 0
            rptr += rstep;                                      // increment pointer by rstep (twice the axis length because it's NPY_CDOUBLE)
            dptr += npts;
      }
...
execute_real_backward(PyObject *a1, double fct)
{
    ...
    PyArrayObject *data = (PyArrayObject *)PyArray_FromAny(a1,
            ...
            NULL);
    if (!data) return NULL;
    int npts = PyArray_DIM(data, PyArray_NDIM(data) - 1);                   // npts = data.shape[-1]
    PyArrayObject *ret = (PyArrayObject *)PyArray_Empty(PyArray_NDIM(data), // ret = np.empty(data.shape, dtype=np.float64)
            PyArray_DIMS(data), PyArray_DescrFromType(NPY_DOUBLE), 0);
    int fail = 0;
    if (!ret) fail=1;
    if (!fail) {
      int nrepeats = PyArray_SIZE(ret)/npts;                                // nrepeats = ret.size // npts (should be 1 for 1d array)
      ...
      if (!fail) {
        for (int i = 0; i < nrepeats; i++) {
          memcpy((char *)(rptr + 1), (dptr + 2), (npts - 1)*sizeof(double));// copy data to ret with len=npts-1 and offsets:
                                                                            // data offset by 2 (one complex element)
                                                                            // ret offset by 1 (one double element)
          rptr[0] = dptr[0];                                                // ret[0] = data[0].real
          if (rfft_backward(plan, rptr, fct)!=0) {fail=1; break;}           // do the ifft with no offsets
          rptr += npts;
          dptr += npts*2;
        }
      }
